...
public class SewedBinaryTree {

    int count, maxLevel;
    Link root;

    public SewedBinaryTree() {
        count = 0;
        root = null;
    }
        ...
        if (current == null) {
            ...
            while (true) {
                ...
                if (current.key > key) {
                    ...
                    if (current.right == null || current.right.level < level) {
                        addable.level = level + 1;
    ...
    private Link getSuccessor(Link deletable) {
        ...
        if ((deletable.right == null || deletable.right.level < level) && (deletable.left == null || deletable.left.level < level)) {
            return null;
        }

        if (deletable.right == null || deletable.right.level < level) {
            level++;
            Link current = deletable.left;
            ...
            while (current.right.right != null && current.right.right.level == level) {
                level++;
                current = current.right;
            }

            Link returnable = current.right;
            if (current.right.left.level > current.level) {
